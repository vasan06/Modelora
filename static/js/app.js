...
function setupUpload() {
  ...
  if (S.isDemo) {
    ...
    if (note) {
      ...
      note.innerHTML = '<div style="background:rgba(0,212,255,.1);border:1px solid rgba(0,212,255,.3);border-radius:6px;padding:8px;font-size:11px;color:var(--accent);margin-top:8px;text-align:center">Demo Mode: Built-in datasets are available. <a href="/login" style="color:var(--primary);text-decoration:underline">Sign up</a> to upload your own data.</div>';
    }
  }
  
  if (!S.isDemo && zone && inp) {
  ...
  document.getElementById('builtin-sel').addEventListener('change', async e => {
    ...
    try {
      const d = await api('POST', '/api/dataset/builtin', { name: e.target.value }, 60000);
      hideLoad();
      if (d.ok) {
...
async function handleFile(file) {
  ...
  const d = await api('POST', '/api/dataset/upload', { content, filename: file.name }, 60000);
  hideLoad();
