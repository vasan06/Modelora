...
def predictions():
    ...
    path = file_store.cache_get(f"{u}:predictions")
    if not path:
        path = os.path.join(config.EXPORT_DIR, f"{file_store.scoped_name(u, name)}_predictions.csv")
...
def processed():
    u = require_auth(request, _s()) or "anon"
    name = file_store.cache_get(f"{u}:name") or "dataset"
    path = file_store.cache_get(f"{u}:processed")
    if not path:
        path = os.path.join(config.PROCESSED_DIR, f"{file_store.scoped_name(u, name)}_processed.csv")
...
def summary():
    ...
    name = file_store.cache_get(f"{u}:name") or "dataset"
    path = file_store.export_summary(json.loads(rj), name, u)
    return send_file(path, as_attachment=True)
